...
class TFNodeContext:
    ...
    封装唯一的元数据信息为每个节点并且提供和spark HDFS的交互
...
def __init__(self, executor_id=0, job_name='', task_index=0, cluster_spec={}, defaultFS='file://', working_dir='.',
             mgr=None):
...
class TFSparkNode(object):
    """
    TFCluster 通过调用 TFSparkNode 来管理 TF集群
    ...
    **This class is not intended for end-users (see TFNode for end-user APIs)**.
    ...
    This also manages a reference to the TFManager "singleton" per executor.
    Since Spark can spawn more than one python-worker per executor, this will reconnect to the "singleton" instance as needed.
    """
...
def _get_manager(cluster_info, host, executor_id):
    ...
    logger.info("Connected to TFSparkNode.mgr on {0}, executor={1}, state={2}".format(host, executor_id, str(
        TFSparkNode.mgr.get('state'))))
    return TFSparkNode.mgr


def run(fn, tf_args, cluster_meta, tensorboard, log_dir, queues, background):
    """
    打包用户提供的函数给 spark mappartitions function
    ...
    Args:
      ...
      :queues: *INTERNAL_USE*  worker服务器的队列
...
def train(cluster_info, cluster_meta, feed_timeout=600, qname='input'):
    ...
    def _train(iter):  # 迭代分区数据
        # get shared queue, reconnecting if necessary
        mgr = _get_manager(cluster_info, util.get_ip_address(), util.read_executor_id())  # 获取 manager
        try:
            queue = mgr.get_queue(qname)  # input 队列
            equeue = mgr.get_queue('error')  # error 队列
        except (AttributeError, KeyError):
            msg = "Queue '{}' not found on this node, check for exceptions on other nodes.".format(qname)
            raise Exception(msg)

        state = str(mgr.get('state'))  # 获取状态
        logger.info("mgr.state={0}".format(state))
        terminating = state == "'terminating'"  # 是否终止
        if terminating:
            logger.info("mgr is terminating, skipping partition")
            count = sum(1 for item in iter)
            logger.info("Skipped {0} items from partition".format(count))  # 剩余多少条没有进入input队列
        else:
            ...
            while (joinThr.isAlive()):  # 会一直阻塞直到队列为空
                if (not equeue.empty()):
                    e_str = equeue.get()
        ...
        if not terminating:
            ...
            if terminating:
                try:
                    ...
                    client.request_stop()  # 客户端向服务端发送终止信号
                    client.close()
