...
"""
  将sparkDataFrame 保存为 tfRecord
  将磁盘上的tfRecord加载为DataFrame
  
  主要方法：
      isLoadedDF： df是否被加载，被加载之后会存入本地变量 loadedDF之中
      saveAsTFRecords： 将df 保存为tfRecords
      loadTFRecords： 将tfRecords加载为df
"""
...
loadedDF = {}       # Stores origin paths of loaded DataFrames (df => path) （key， value）


def isLoadedDF(df):
  # 判断df是否是 loadtfrecords方法加载，应为只有这个方法会将结果df写入 loadeddf字典
...
def loadTFRecords(sc, input_dir, binary_features=[]):
  ...
  schema = infer_schema(example, binary_features) # 推测出dataframe 的元数据信息
...
def toTFExample(dtypes):
  # 转换rdd row 为 tf.train.Example bytestring
  ...
  def _toTFExample(iter):
    ...
    results = []
    for row in iter:
      features = dict([_toTFFeature(name, dtype, row) for name, dtype in dtypes])   # (name, tf.train.Feature)
...
def infer_schema(example, binary_features=[]):
  # 给一个tf 训练样本，推测出sparkdataframe的元数据
