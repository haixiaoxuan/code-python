...
"""
  这个示例演示了如何利用Spark从SavedModel进行并行推理。
  通常，您可以使用TensorFlowOnSpark来形成一个TensorFlow集群，用于训练和推断。
  但是，在某些情况下，您可能拥有一个SavedModel，而不需要定义推理的原始代码图。
  在这些情况下，我们可以使用Spark在每个执行器上实例化单节点TensorFlow实例，
  其中每个执行器可以独立地加载输入数据上的模型和推理。
  注意:这个特殊的例子演示了' tf.data '的用法。读取输入数据以进行推理，
  但是也可以只使用Spark的tfrecord的RDD。
"""
